...
export async function pantallaEvaluacion(hoja) {
  ...
  async function importarNotas() {
    ...
    const n = await importarPlanilla({
      titulo: "Importar notas desde una planilla",
      bajada:
        "Una fila por bombero y una columna por categoría, del 1 al 10. " +
        "Las columnas que falten o queden vacías no se tocan: la nota que ya " +
        "tenía se conserva.",
      columnas: [
        { clave: "legajo", requerida: true, alias: ["nro legajo", "n legajo"] },
        ...CATEGORIAS.map(c => ({
          clave: c.k,
          alias: [c.label, c.label.replace(/\s+/g, "")],
          ayuda: c.label + " (1 a 10)"
        }))
      ],
      ...
      validar: (f, { numero }) => {
        ...
        for (const c of CATEGORIAS) {
          const bruto = f[c.k];
          if (!bruto || !String(bruto).trim()) continue;   // vacío = no tocar
          const v = numero(bruto);
          if (v === null)
            return { ok: false, motivo: `${c.label}: «${bruto}» no es un número` };
          if (!Number.isInteger(v) || v < 1 || v > 10)
            return { ok: false, motivo: `${c.label}: ${v} está fuera de la escala 1–10` };
          campos[c.k] = v;
        }

        if (!Object.keys(campos).length)
          return { ok: false, motivo: "no trae ninguna nota" };

        vistos.add(b.id);
        return { ok: true, valor: { legajo, nombre: b.nombre, ...campos, _id: b.id } };
      },

      // Una llamada por bombero: `guardarEvaluacion` es un UPDATE por
      // (periodo, bombero) y no hay upsert masivo del lado del servidor. Son
      // pocas filas y la barra de progreso lo hace tolerable.
      aplicar: async (validos, informar) => {
        let hechas = 0;
        for (const v of validos) {
          const { legajo, nombre, _id, _fila, ...campos } = v;
          await guardarEvaluacion(estado.periodo.id, _id, campos);
          informar(++hechas, validos.length);
        }
      }
    });

    if (n) { avisar(`${n} evaluación(es) importadas`, "ok"); pantallaEvaluacion(hoja); }
  }
  ...
  montar(limpiar(hoja), 
    el("div", { class: "cabecera" },
      ...
      el("div", { class: "acciones" },
        ...
        cerrado ? null : el("button", { class: "btn sec", onclick: importarNotas },
                            "Importar planilla"),
        btnGuardar
      )
    ),
      ...
      el("div", { class: "scroll-tabla" },
        el("table", {},
          el("thead", {}, el("tr", {},
            ...
            el("th", {}, "Bombero"),
            CATEGORIAS.map(c => el("th", { class: "cen", title: `${c.label} — ${c.ayuda}` },
              c.corto)),
            el("th", {}, "")
          )),
          cuerpo
